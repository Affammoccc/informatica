...
def sessione(primo_appello,secondo_appello):
    ...
    sessione_output=sorted(sessione, key=lambda item : (item["voto"], item["cognome"]))
    file=open("sessione.csv","w",encoding="utf-8")
    file.write("nome,cognome,voto,anno_nascita,media_esami_precedenti\n")
    for elementio in sessione_output:
        valori = list(elementio.values())
        file.write(f"{valori[0]},{valori[1]},{valori[2]},{valori[3]},{valori[4]}\n")    
    return 
